...
import { analyzeActivationCall } from '@/lib/services/activation-analysis';
...
export const POST = withAuth(async (_request: NextRequest, auth, context?: RouteHandlerContext) => {
  ...
  try {
    ...
    const { meeting_title, occurred_at, owner_id, fathom_share_url } = engRow.rows[0];
    ...
    for (const email of ownerEmails.length > 0 ? ownerEmails : [undefined]) {
      ...
      match = meetings.find(m => m.share_url === fathom_share_url) ?? null;
    }
    ...
    if (!match) {
      ...
      match = fallbackMeetings.find(m => m.share_url === fathom_share_url) ?? null;
    }
